...
%
% Things to improve:
% Offload event creation for after the task
% Examples include only running datetime once and computing
% precompute video before showing it.
...
% -------------------------------------------------------------------------
        case 99
            ...
            for i = countdown_from:-1:1
                ...
                Screen('TextFont', window_1, cfg.format.font);
...
% -------------------------------------------------------------------------
        case 2
            ...
            drawCross(window_1, W, H);
            tFixation = Screen('Flip', window_1);
            NetStation('Event','EVEN',tFixation, 0.001, 'cros',2); NetStation('FlushReadbuffer');            
        ...
        case 3
            ...
            try
                ...
                if tex > 0  % If a valid texture was returned
                    ...
                    tMovie = Screen('Flip', window_1);  % Update the screen to show the first frame
                    % -------------------------------------------
...
% -------------------------------------------------------------------------
        case 5 
            ...
            while ~clicked_in_circle
                ...
                NetStation('Event','EVEN',ValenceTime, 0.001, 'vale',5); NetStation('FlushReadbuffer');                                                        
                eventOnsets(event_) = GetSecs() - start_exp;
                eventTime{event_}   = datetime('now');
